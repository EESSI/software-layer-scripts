...
DEBUG="getent hosts github.com && echo && echo DEBUG && curl -v --connect-to ::140.82.121.4 https://github.com && ldd $(which curl) | grep -E "libc|nss" && ldd $(which git)  | grep -E "libc|nss" && curl -I https://github.com && env | grep -i proxy && git config --list | grep proxy && git ls-remote https://github.com/EESSI/test-suite/"
