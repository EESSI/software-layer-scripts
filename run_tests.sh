...
DEBUG="getent hosts github.com && echo && echo DEBUG && ldd $(ls $(command -v curl)) | grep -E 'libc|nss' && env && git config --list | grep proxy && git ls-remote https://github.com/EESSI/test-suite/ && curl -I https://github.com && env | grep -i proxy && git config --list | grep proxy && git ls-remote https://github.com/EESSI/test-suite/"
